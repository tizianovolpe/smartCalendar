...
function calGen(anno,prefs){
    ...
    var pasquaMeseGiorno = calcolaPasqua(anno);
    
    var thisCalSettings = calSettings;
	
	//cancello da calSettings il 29 febbrario nel caso in cui l'anno non sia bisestile
	var bisestile = bisestileCalc(anno);
    if(!bisestile){
        
        delete thisCalSettings['mesi']['feb']['santi']['29'];
        thisCalSettings['mesi']['feb']['numero giorni'] = '28';
    }
	...
	*/
	...
	var day2day0 = 0;
	for (i=thisCalSettings.utility.zero.anno; i<anno; i++){
	...
	*/
	...
	for(mese in thisCalSettings.mesi ){
		
		mesi[mese] = {};
		mesi[mese]['nome'] = cut(thisCalSettings.mesi[mese]['nome'],prefs.nomeMese);
		mesi[mese]['giorni'] = {};
		...
		for(santo in thisCalSettings.mesi[mese]['santi']){
			...
			mesi[mese]['giorni'][numero]['giorno'] = cut(thisCalSettings.giorni[day],prefs.nomeGiorno);
			mesi[mese]['giorni'][numero]['santo'] = thisCalSettings.mesi[mese]['santi'][santo];
            ...
            if(thisCalSettings.festivi[mese][numero] ){
                mesi[mese]['giorni'][numero]['santo'] = thisCalSettings.festivi[mese][numero];
                mesi[mese]['giorni'][numero]['festivo'] = true;
            }
            ...
            */
			...
			if(day==thisCalSettings.giorni.length-1) {
				day=0;
			}else{
				day ++;
			}
    ...
    for (mese in mesi){
        for(giorno in mesi[mese]['giorni']){
            ...
            for(festaMobile in thisCalSettings['giorni mobili'] ){
                
                //starter descrive il giorno di partenza da cui calcolare la festa mobile
                var starter;
                if(thisCalSettings['giorni mobili'][festaMobile]['start']=='pasqua'){
                    starter = pasqua;
                }else if(thisCalSettings['giorni mobili'][festaMobile]['start']=='IVavvento'){
                    starter = IVavvento;
                }else{
                    alert('attenzione stai tentando di calcolare una festa mobile non prevista');
                }
                
                var giornoMobile = starter + thisCalSettings['giorni mobili'][festaMobile]['giorno'];
                
                if(giornoCorrente == giornoMobile){
                    mesi[mese]['giorni'][giorno]['santo'] = thisCalSettings['giorni mobili'][festaMobile]['nome'];
                    
                    if(thisCalSettings['giorni mobili'][festaMobile]['festivo'] == true){
